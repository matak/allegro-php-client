...
sami_base_dir="${docs_base_dir}/sami"
sami_dir="${sami_base_dir}/${docs_revision}"
sami="$( mktemp )"
sami_config="$( mktemp )"
sami_cmd=(
    php "${sami}"
...
\$iterator = Finder::create()
    ->files()
    ->name('*.php')
    $(for p in "${docs_gen_ignore_dirs[@]}"; do echo "->exclude('$p')"; done )
    ->in('$docs_gen_src_dir')
;
...
wget http://get.sensiolabs.org/sami.phar -O "$sami" || exit 1

rm -rf "$sami_dir" || true
mkdir -p "$sami_dir"

echo "Running Sami: " "${sami_cmd[@]}"
"${sami_cmd[@]}" || exit 33

ln -f -s "$( basename "$sami_dir" )" "${sami_base_dir}/latest"
